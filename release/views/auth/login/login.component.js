...
var DnLoginComponent = /** @class */ (function () {
    ...
    function DnLoginComponent(router, authService, _fb, notificationService, loginConfigService, storageService, _compiler) {
        this.router = router;
    ...
    DnLoginComponent.prototype.login = function (data, isValid) {
        ...
        if (!isValid)
            return;
        this.submitted = true;
        this.authService.login(data.mail, data.password).subscribe(function (res) {
            if (res.isSuccess) {
                var user = res.data;
                _this._compiler.clearCache();
    ...
    DnLoginComponent.prototype.ngOnInit = function () {
        if (config.debugState) {
            this.loginForm = this._fb.group({
                mail: ['gtb@gtb.gov.tr', [Validators.required, Validators.minLength(3)]],
